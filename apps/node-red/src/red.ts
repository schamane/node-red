...
export const main = async () => {
  ...
  if (parsedArgs.settings) {
  ...
  } else if (fs.existsSync(path.join(process.env.NODE_RED_HOME, '.config.json'))) {
    ...
    const userDir = parsedArgs.userDir || path.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH, '.node-red');
    const userSettingsFile = path.join(userDir, 'settings.js');
    if (fs.existsSync(userSettingsFile)) {
      // $HOME/.node-red/settings.js exists
      settingsFile = userSettingsFile;
    } else {
      const defaultSettings = path.join(__dirname, 'settings.js');
  ...
  try {
    ...
    await import(settingsFile).then((s) => {
  ...
  httpsPromise
    .then(function (httpsStartupHttps) {
      if (httpsStartupHttps) {
        server = https.createServer(httpsStartupHttps, function (req, res) {
          app(req, res);
        });
