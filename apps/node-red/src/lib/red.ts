...
import { init, log, util, events } from '@node-red/util';
...
export default {
  ...
  start() {
    ...
    //
    const startPromise = runtime.start().then(function () {
      if (apiEnabled) {
        return api.start();
      }
    });
    ...
    startPromise.then = function (resolve, reject) {
      ...
      inner.otherwise = function (cb) {
        log.error('**********************************************');
        log.error('* Deprecated call to RED.start().otherwise() *');
        log.error('* This will be removed in Node-RED 2.x       *');
        log.error('* Use RED.start().catch() instead            *');
        log.error('**********************************************');
