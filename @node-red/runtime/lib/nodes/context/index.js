...
function logUnknownStore(name) {
    if (name) {
        ...
        if (count === 0) {
            util_1.log.warn(util_1.log._('context.unknown-store', { name }));
            count++;
...
function init(_settings) {
    ...
    stores._ = (0, memory_js_1.default)();
    defaultStore = 'memory';
}
...
function load() {
    return new Promise(function (resolve, reject) {
        ...
        if (plugins && Object.keys(plugins).length > 0) {
            ...
            for (const pluginName in plugins) {
                if (plugins.hasOwnProperty(pluginName)) {
                    ...
                    if (!/^[a-zA-Z0-9_]+$/.test(pluginName)) {
                        return reject(new Error(util_1.log._('context.error-invalid-module-name', { name: pluginName })));
                    }
                    ...
                    if (pluginName === 'default' && typeof plugins[pluginName] === 'string') {
                        ...
                        if (!plugins.hasOwnProperty(plugins[pluginName])) {
                            return reject(new Error(util_1.log._('context.error-invalid-default-module', { storage: plugins.default })));
                        }
                        defaultIsAlias = true;
                        continue;
                    }
                    ...
                    if (plugins[pluginName].hasOwnProperty('module')) {
                        ...
                        if (typeof plugins[pluginName].module === 'string') {
                            ...
                            try {
                                ...
                                return reject(new Error(util_1.log._('context.error-loading-module2', { module: plugins[pluginName].module, message: err.toString() })));
                            }
                        }
                        else {
                        ...
                        try {
                            ...
                            util_1.log.info(util_1.log._('context.log-store-init', { name: pluginName, info: 'module=' + moduleInfo }));
                        }
                        catch (err) {
                            return reject(new Error(util_1.log._('context.error-loading-module2', { module: pluginName, message: err.toString() })));
                        }
                    }
                    else {
                        // Plugin does not specify a 'module'
                        return reject(new Error(util_1.log._('context.error-module-not-defined', { storage: pluginName })));
                    }
                }
            }
            ...
            util_1.log.info(util_1.log._('context.log-store-init', { name: 'default', info: 'module=memory' }));
        ...
        throw new Error(util_1.log._('context.error-loading-module', { message: err.toString() }));
    });
}
function copySettings(config, localCopySettings) {
    ...
    copy.forEach(function (setting) {
        config.settings[setting] = (0, clone_1.default)(localCopySettings[setting]);
    });
}
        ...
        insertSeedValues = function (keys, values) {
            if (!Array.isArray(keys)) {
                if (values[0] === undefined) {
                    try {
                        values[0] = util_1.util.getObjectProperty(seed, keys);
                    }
                    catch (err) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    if (values[i] === undefined) {
                        try {
                            values[i] = util_1.util.getObjectProperty(seed, keys[i]);
                        }
                        catch (err) {
                            if (err.code === 'INVALID_EXPR') {
                                throw err;
        ...
        get: {
            value(key, storage, callback) {
                ...
                if (!Array.isArray(key)) {
                    const keyParts = util_1.util.parseContextStore(key);
                    key = keyParts.key;
                    if (!storage) {
                ...
                if (callback) {
                    ...
                    if (seed) {
                        if (Array.isArray(key)) {
                            ...
                            try {
                                results = util_1.util.getObjectProperty(seed, key);
                            }
                            catch (err) {
                                if (err.code === 'INVALID_EXPR') {
                                    throw err;
