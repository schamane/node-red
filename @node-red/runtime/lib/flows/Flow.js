...
exports.Flow = exports.init = void 0;
...
class Flow {
    ...
    constructor(parent, globalFlow, flow) {
        ...
        this.context = index_js_1.default.getFlowContext(this.id, this.parent.id);
    }
    /**
        ...
        while (configNodes.length > 0) {
            ...
            if (!this.activeNodes[id]) {
                if (node.d !== true) {
                    ...
                    if (readyToCreate) {
                        newNode = util_js_1.default.createNode(this, node);
                        if (newNode) {
            ...
            if (this.flow.nodes.hasOwnProperty(id)) {
                ...
                if (node.d !== true) {
                    if (!node.subflow) {
                        ...
                        try {
                            const subflowDefinition = this.flow.subflows[node.subflow] || this.global.subflows[node.subflow];
                            // console.log("NEED TO CREATE A SUBFLOW",id,node.subflow);
                            this.subflowInstanceNodes[id] = true;
                            const subflow = (0, subflow_js_1.createSubflow)(this, this.global, subflowDefinition, node);
        ...
        for (id in this.activeNodes) {
            if (this.activeNodes.hasOwnProperty(id)) {
                ...
                if (node.type === 'catch') {
                    ...
                    if (node.scope) {
                        node.scope.forEach((key) => {
                            this.completeNodeMap[key] = this.completeNodeMap[key] || [];
                            this.completeNodeMap[key].push(node);
                        });
                    }
                }
            }
    ...
    getNode(id, cancelBubble) {
        ...
        if ((this.flow.configs && this.flow.configs[id]) ||
            (this.flow.nodes && this.flow.nodes[id] && this.flow.nodes[id].type.substring(0, 8) !== 'subflow:')) {
    ...
    getGroupEnvSetting(node, group, name) {
        if (group) {
            ...
            if (group.credentials === undefined) {
                group.credentials = credentials_js_1.default.get(group.id) || {};
            }
                ...
                if (group.env) {
                    ...
                    if (env) {
                        ...
                        if (type !== 'env' || value !== name) {
                            ...
                            try {
                                const val = util_1.util.evaluateNodeProperty(value, type, node, null, null);
                                return [
                                    {
                                        val
                                    },
    ...
    getSetting(key) {
        ...
        if (flow.credentials === undefined) {
            flow.credentials = credentials_js_1.default.get(flow.id) || {};
        }
        if (flow.env) {
                ...
                if (env) {
                    ...
                    if (type !== 'env' || value !== key) {
                        ...
                        try {
                            ...
                            const val = util_1.util.evaluateNodeProperty(value, type, null, null, null);
                            return val;
                        }
                        catch (e) {
                            this.error(e);
    ...
    handleStatus(node, statusMessage, reportingNode, muteStatusEvent) {
        ...
        if (this.id === 'global' && node.users) {
            ...
            this.statusNodes.forEach(function (targetStatusNode) {
                ...
                const message = {
                    status: (0, clone_1.default)(statusMessage)
                };
    ...
    handleError(node, logMessage, msg, reportingNode) {
        ...
        if (this.id === 'global' && node.users) {
            ...
            this.catchNodes.forEach(function (targetCatchNode) {
                ...
                let errorMessage;
                if (msg) {
                    errorMessage = util_1.util.cloneMessage(msg);
                }
                else {
                    errorMessage = {};
                }
    ...
    handleComplete(node, msg) {
        if (this.completeNodeMap[node.id]) {
            ...
            this.completeNodeMap[node.id].forEach((completeNode, index) => {
                toSend = util_1.util.cloneMessage(msg);
...
function handleOnSend(flow, sendEvents, reportError) {
    // onSend - passed an array of SendEvent objects. The messages inside these objects are exactly what the node has passed to node.send - meaning there could be duplicate references to the same message object.
    util_1.hooks.trigger('onSend', sendEvents, (err) => {
        if (err) {
...
function handlePreRoute(flow, sendEvent, reportError) {
    // preRoute - called once for each SendEvent object in turn
    util_1.hooks.trigger('preRoute', sendEvent, (err) => {
        if (err) {
            ...
            if (sendEvent.destination.node) {
                if (sendEvent.cloneMessage) {
                    sendEvent.msg = util_1.util.cloneMessage(sendEvent.msg);
                }
...
function handlePreDeliver(flow, sendEvent, reportError) {
    // preDeliver - the local router has identified the node it is going to send to. At this point, the message has been cloned if needed.
    util_1.hooks.trigger('preDeliver', sendEvent, (err) => {
        if (err) {
            reportError(err, sendEvent);
            return;
        }
            ...
            // postDeliver - the message has been dispatched to be delivered asynchronously (unless the sync delivery flag is set, in which case it would be continue as synchronous delivery)
            util_1.hooks.trigger('postDeliver', sendEvent, function (err1) {
                if (err1) {
                    reportError(err1, sendEvent);
                }
            });
        }
    });
