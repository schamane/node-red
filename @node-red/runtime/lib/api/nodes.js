...
const api = {
    ...
    addModule(opts) {
        if (!runtime.settings.available()) {
            ...
            const err = new Error('Settings unavailable');
        ...
        try {
            ...
            if (!node) {
                ...
                return putNode(node, enabled)
                    ...
                    .catch(function (err) {
                    runtime.log.audit({ event: 'nodes.info.set', id, enabled, error: err.code || 'unexpected_error', message: err.toString() }, opts.req);
                    err.status = 400;
                    throw err;
                });
            }
            ...
            runtime.log.audit({ event: 'nodes.info.set', id, enabled, error: error.code || 'unexpected_error', message: error.toString() }, opts.req);
            error.status = 400;
            throw error;
        }
    },
    ...
    async getIcon(opts) {
        ...
        if (iconPath) {
            return fs_extra_1.default.readFile(iconPath).catch((err) => {
                err.status = 400;
                throw err;
            });
        }
    ...
    async getModuleResource(opts) {
        ...
        if (resourcePath) {
            return fs_extra_1.default.readFile(resourcePath).catch((err) => {
                if (err.code === 'EISDIR') {
                    return null;
                }
                err.status = 400;
