...
import { util as redUtil, log as Log, hooks } from '@node-red/util';
...
Node.prototype._emitInput = function (arg) {
  ...
  hooks.trigger('onReceive', receiveEvent, (err) => {
    if (err) {
      ...
      hooks.trigger('postReceive', receiveEvent, (err5) => {
        if (err5) {
          node.error(err5);
        }
      });
...
function log_helper(self: typeof Node, level, msg) {
  const o: any = {
    level,
    id: self.id,
    type: self.type,
...
function inspectObject(flow) {
  try {
    ...
    const propList = [...properties.keys()].map((item: string) => `${item}[${(typeof flow[item])[0]}]`);
    propList.sort();
    const result = [];
    let line = '';
