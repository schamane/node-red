...
import externalAPI from './api/index.js';
import redNodes from './nodes/index.js';
...
async function getVersion() {
  if (!version) {
    ...
    version = await import(path.join(__dirname, '..', 'package.json')).then(({ version }) => version);
  ...
  return i18n
    ...
    .then(function () {
      ...
      if (settings.UNSUPPORTED_VERSION) {
        log.error('*****************************************************************');
        log.error(
          '* ' + log._('runtime.unsupported_version', { component: 'Node.js', version: process.version, requires: '>=8.9.0' }) + ' *'
        );
        log.error('*****************************************************************');
      ...
      return redNodes.load().then(function () {
        ...
        if (settings.httpStatic && settings.httpStatic.length) {
          for (let si = 0; si < settings.httpStatic.length; si++) {
            const p = path.resolve(settings.httpStatic[si].path);
            const r = settings.httpStatic[si].root || '/';
...
function reinstallModules(moduleList) {
  ...
  if (settings.autoInstallModulesRetry) {
