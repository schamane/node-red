...
function checkFilePairExist(privateKeyFilePath, publicKeyFilePath) {
  return Promise.all([
    fs.access(privateKeyFilePath, ((fs.constants || fs) as any).R_OK),
    fs.access(publicKeyFilePath, ((fs.constants || fs) as any).R_OK)
  ]);
}
...
function generateSSHKeyPair(name, privateKeyPath, comment, password, size) {
  ...
  return keygen
    ...
    .catch(function (err) {
      log.log(`[SSHKey generation] error: ${inspect(err)}`);
      throw err;
    });
}
...
function getPrivateKeyPath(username, name) {
  ...
  try {
    fs.accessSync(privateKeyFilePath, ((fs.constants || fs) as any).R_OK);
  } catch (err) {
