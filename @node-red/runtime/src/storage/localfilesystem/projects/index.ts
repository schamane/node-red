...
function init(_settings, _runtime) {
  ...
  const ffExt = fspath.extname(flowsFullPath);
  const ffBase = fspath.basename(flowsFullPath, ffExt);
  ...
  if (projectsEnabled) {
    return sshTools.init(settings).then(function () {
      gitTools.init(_settings).then(function (gitConfig: any) {
          ...
          if (!settings.readOnly) {
            return (
              fs
                ...
                .then(function (globalSettings: any) {
                  ...
                  if (settings.flowFile) {
                    ...
                    if (globalSettings.projects.projects.hasOwnProperty(settings.flowFile)) {
                      ...
                      try {
                        const stat = fs.statSync(fspath.join(projectsDir, settings.flowFile));
                  ...
                  if (saveSettings) {
                    return storageSettings.saveSettings(globalSettings) as any;
                  }
                })
            );
          }
...
function listProjects() {
  return fs.readdir(projectsDir).then(function (fns) {
    ...
    fns
      ...
      .filter(function (fn) {
        const fullPath = fspath.join(projectsDir, fn);
        if (fn[0] !== '.') {
...
function loadProject(name) {
  const fullPath = fspath.resolve(fspath.join(projectsDir, name));
  let projectPath = name;
  if (projectPath.indexOf(fspath.sep) === -1) {
    projectPath = fullPath;
  } else {
    // Ensure this project dir is under projectsDir;
    const relativePath = fspath.relative(projectsDir, fullPath);
...
function deleteProject(user, name) {
  if (activeProject && activeProject.name === name) {
    ...
    (e as any).code = 'cannot_delete_active_project';
    throw e;
  }
  const projectPath = fspath.join(projectsDir, name);
  const relativePath = fspath.relative(projectsDir, projectPath);
  if (/^\.\./.test(relativePath)) {
    throw new Error('Invalid project name');
  }
  ...
  return runtime.nodes
    ...
    .then(function () {
      ...
      return runtime.nodes
        .loadFlows(true)
        .then(function () {
          events.emit('runtime-event', { id: 'project-update', payload: { project: activeProject.name, action } });
        })
        .catch(function (err) {
...
function saveFlows(flows, user) {
  ...
  if (activeProject && activeProject.isMerging()) {
    ...
    (error as any).code = 'git_merge_conflict';
    throw error;
  }
      ...
      if (workflowMode === 'auto') {
        return activeProject.stageFile([flowsFullPath, credentialsFile]).then(() => {
          return activeProject.status(user, false).then((result) => {
            ...
            if (items.findIndex((item: any) => item.status === 'M ') < 0) {
              return Promise.resolve();
            }
