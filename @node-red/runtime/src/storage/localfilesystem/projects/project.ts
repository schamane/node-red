...
import defaultFileSet from './defaultFileSet.js';
import sshKeys from './ssh/index.js';
import { log, events } from '@node-red/util';
import authCache from './git/authCache.js';
...
Project.prototype.load = function () {
  ...
  return checkProjectFiles(project).then(function (missingFiles) {
    ...
    if (missingFiles.indexOf('package.json') === -1) {
      ...
      promises.push(
        fs
          .readFile(fspath.join(project.path, project.paths['package.json']), 'utf8')
      ...
      if (missingFiles.indexOf('README.md') === -1) {
        ...
        promises.push(
          fs
            .readFile(fspath.join(project.path, project.paths['README.md']), 'utf8')
            .then(function (content) {
...
Project.prototype.initialise = function (user, data) {
  const project = this;
  const files = Object.keys(defaultFileSet);
  const promises = [];
  ...
  if (data.hasOwnProperty('files')) {
    if (data.files.hasOwnProperty('flow') && data.files.hasOwnProperty('credentials')) {
      ...
      const flowFilePath = fspath.join(project.path, project.files.flow);
  ...
  for (const file in defaultFileSet) {
    if (defaultFileSet.hasOwnProperty(file)) {
      const path = fspath.join(project.path, file);
...
Project.prototype.update = async function (user, data) {
  ...
  if (data.credentialSecret && data.credentialSecret !== this.credentialSecret) {
    ...
    if (
      ...
      (e as any).code = 'missing_current_credential_key';
      throw e;
    }
  ...
  if (data.hasOwnProperty('files')) {
    ...
    if (
      data.files.hasOwnProperty('package') &&
      (data.files.package !== fspath.join(this.paths.root, 'package.json') || !this.paths['package.json'])
    ) {
      ...
      if (fs.existsSync(fspath.join(this.path, this.paths['package.json']))) {
        ...
        const newPackage = defaultFileSet['package.json'](this);
    ...
    if (
      data.files.hasOwnProperty('flow') &&
    ...
    if (
      data.files.hasOwnProperty('credentials') &&
      ...
      remoteNames.forEach(function (name) {
        if (data.git.remotes[name].removed) {
          ...
          modifyRemotesPromise = modifyRemotesPromise.then(function () {
            gitTools.removeRemote(project.path, name);
          });
        } else {
          if (data.git.remotes[name].url) {
            remotesChanged = true;
            modifyRemotesPromise = modifyRemotesPromise.then(function () {
              gitTools.addRemote(project.path, name, data.git.remotes[name]);
            });
          }
  ...
  if (saveREADME) {
    promises.push(util.writeFile(fspath.join(this.path, this.paths['README.md']), this.description).catch());
    modifiedFiles.push('README.md');
  }
  if (savePackage) {
    promises.push(
      fs
        .readFile(fspath.join(this.path, this.paths['package.json']), 'utf8')
        .then((content) => {
          let currentPackage = {};
          try {
          ...
          return util.writeFile(fspath.join(project.path, this.paths['package.json']), JSON.stringify(this.package, undefined, 4));
        })
        .catch((err) => {
          //
        })
...
Project.prototype.getFile = function (filePath, treeish) {
  ...
  const fullPath = fspath.join(this.path, filePath);
...
Project.prototype.status = function (user, includeRemote) {
  ...
  const completeStatus = function (fetchError) {
    const promises = [gitTools.getStatus(self.path), fs.exists(fspath.join(self.path, '.git', 'MERGE_HEAD'), undefined)];
    return Promise.all(promises)
...
Project.prototype.push = function (user, remoteBranchName, setRemote) {
  ...
  return gitTools.push(
    this.path,
    remote.remote || this.currentRemote,
    remote.branch,
    setRemote,
    authCache.get(this.name, this.remotes[remote.remote || this.currentRemote].fetch, username)
  );
};
...
Project.prototype.pull = function (user, remoteBranchName, setRemote, allowUnrelatedHistories) {
  ...
  if (setRemote) {
    return gitTools.setUpstream(this.path, remoteBranchName).then(function () {
      ...
      return gitTools.pull(
        self.path,
        null,
        null,
        allowUnrelatedHistories,
        authCache.get(self.name, self.remotes[self.currentRemote].fetch, username),
        getGitUser(user)
      );
    });
  }
  ...
  return gitTools.pull(
    ...
    remote.branch,
    allowUnrelatedHistories,
    authCache.get(this.name, this.remotes[remote.remote || self.currentRemote].fetch, username),
    getGitUser(user)
  );
};

Project.prototype.resolveMerge = function (file, resolutions) {
  const filePath = fspath.join(this.path, file);
...
Project.prototype.fetch = function (user, remoteName) {
  ...
  if (remoteName) {
    return gitTools
      .fetch(project.path, remoteName, authCache.get(project.name, project.remotes[remoteName].fetch, username))
      .catch(function (err) {
...
function getCredentialsFilename(filename) {
  ...
  // TODO: DRY - ./index.js
  const ffDir = fspath.dirname(filename);
  const ffExt = fspath.extname(filename);
  const ffBase = fspath.basename(filename, ffExt);
  return fspath.join(ffDir, ffBase + '_cred' + ffExt);
}
function getBackupFilename(filename) {
  // TODO: DRY - ./index.js
  filename = filename || 'undefined';
  const ffName = fspath.basename(filename);
  const ffDir = fspath.dirname(filename);
...
function checkProjectExists(projectPath) {
  return fs.pathExists(projectPath).then(function (exists) {
    if (!exists) {
      ...
      (e as any).code = 'project_not_found';
      const name = fspath.basename(projectPath);
      (e as any).project = name;
      throw e;
    }
  });
}
function createDefaultProject(user, project) {
  const projectPath = fspath.join(projectsDir, project.name);
    ...
    if (project.files) {
      if (project.files.flow && !/\.\./.test(project.files.flow)) {
        ...
        if (project.migrateFiles) {
          const baseFlowFileName = project.files.flow || fspath.basename(project.files.oldFlow);
          const baseCredentialFileName = project.files.credentials || fspath.basename(project.files.oldCredentials);
          ...
          promises.push(
            runtime.nodes.exportCredentials().then(function (creds) {
              let credentialData;
...
function checkProjectFiles(project) {
  ...
  for (const file in defaultFileSet) {
    if (defaultFileSet.hasOwnProperty(file)) {
      (function (f) {
        promises.push(
          fs.stat(fspath.join(project.path, project.paths.root, f)).catch((err) => {
            missing.push(f);
          })
        );
      })(file);
        ...
        .then(function () {
          if (metadata.git && metadata.git.remotes && metadata.git.remotes.origin) {
            ...
            if (originRemote.hasOwnProperty('username') && originRemote.hasOwnProperty('password')) {
              authCache.set(project, originRemote.url, username, {
