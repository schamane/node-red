          ...
          try {
            ...
            fs.copy(backupPath, path, function (backupCopyErr) {
              if (backupCopyErr) {
                ...
                log.warn(log._('storage.localfilesystem.restore-fail', { message: backupCopyErr.toString(), type }));
                resolve([]);
              } else {
      ...
      .then(() => {
        ...
        return new Promise(function (resolve, reject) {
          fs.rename(tempFile, path, (err) => {
            if (err) {
              log.warn(log._('storage.localfilesystem.fsync-fail', { path, message: err.toString() }));
              return reject(err);
            }
