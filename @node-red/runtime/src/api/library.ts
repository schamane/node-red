...
const api = {
  ...
  getEntry(opts) {
    return runtime.library
      .getEntry(opts.library, opts.type, opts.path)
      ...
      .catch(function (err) {
        if (err) {
          runtime.log.warn(
            runtime.log._('api.library.error-load-entry', {
  ...
  saveEntry(opts) {
    return runtime.library
      .saveEntry(opts.library, opts.type, opts.path, opts.meta, opts.body)
      .then(function () {
      ...
      .catch(function (err) {
        ...
        runtime.log.audit(
          { event: 'library.set', type: opts.type, path: opts.path, error: 'unexpected_error', message: err.toString() },
          opts.req
        );
        const error = new Error() as any;
        error.status = 400;
