...
import { deprecated, checkFlowDependencies, getNodeInfo, get } from '@node-red/registry';
import context from '../nodes/context/index.js';
import credentials from '../nodes/credentials.js';
import flowUtil from './util.js';
import { events, util as redUtil } from '@node-red/util';

// TODO: import order shouldnt affect instances Now util should be imported befor flow

import { Flow, init as initFlow } from './flow.js';
...
function setFlows(_config, _credentials, type, muteLog, forceStart, user?) {
  ...
  if (type === 'load') {
    ...
    if (_credentials) {
      ...
      const credsDirty = credentials.dirty();
...
async function start(type?, diff?, muteLog?, isDeploy?) {
  ...
  try {
    await checkFlowDependencies(activeConfig.flows);
  } catch (err) {
  ...
  events.emit('flows:starting', { config: activeConfig, type, diff });
  ...
  if (type === 'full') {
    ...
    if (!activeFlows.global) {
      ...
      activeFlows.global = new Flow(flowAPI, activeFlowConfig);
    }
    ...
    for (id in activeFlowConfig.flows) {
      if (activeFlowConfig.flows.hasOwnProperty(id)) {
        if (!activeFlowConfig.flows[id].disabled && !activeFlows[id]) {
          ...
          activeFlows[id] = new Flow(flowAPI, activeFlowConfig, activeFlowConfig.flows[id]);
  ...
  events.emit('flows:started', { config: activeConfig, type, diff });
  // Deprecated event
...
function stop(type?, diff?, muteLog?, isDeploy?) {
  ...
  events.emit('flows:stopping', { config: activeConfig, type, diff });
  ...
  return Promise.all(promises).then(function () {
    ...
    events.emit('flows:stopped', { config: activeConfig, type, diff });
...
function checkTypeInUse(id) {
  ...
  if (!nodeInfo) {
    ...
    const nodesInUse = [];
    (nodeInfo.types as any[]).forEach(function (t) {
      if (inUse[t]) {
        nodesInUse.push(t);
      }
    });
    if (nodesInUse.length > 0) {
      ...
      const err = new Error(log._('nodes.index.type-in-use', { msg })) as any;
  ...
  for (const id in activeFlowConfig.allNodes) {
    if (activeFlowConfig.allNodes.hasOwnProperty(id)) {
      ...
      if (node.type !== 'tab' && node.type !== 'subflow') {
        ...
        if ((subflowDetails && !activeFlowConfig.subflows[subflowDetails[1]]) || (!subflowDetails && !get(node.type))) {
...
function addFlow(flow, user) {
  ...
  let newConfig = clone(activeConfig.flows);
