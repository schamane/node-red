...
import { util as redUtil, log as Log } from '@node-red/util';
import { createModuleInstance } from './subflow.js';
import { get, getNodeInfo } from '@node-red/registry';
...
function createNode(flow, config) {
  ...
  try {
    const nodeTypeConstructor = get(type);
    if (typeof nodeTypeConstructor === 'function') {
      const conf = clone(config);
      delete conf.credentials;
      try {
        Object.defineProperty(conf, '_module', { value: getNodeInfo(type), enumerable: false, writable: true });
      ...
      const instanceConfig = clone(config);
      ...
      const subflow = createModuleInstance(nodeTypeConstructor.type, flow, flow.global, subflowInstanceConfig, instanceConfig);
...
function parseConfig(config) {
  ...
  config.forEach(function (n) {
    if (n.type !== 'subflow' && n.type !== 'tab' && n.type !== 'group') {
      ...
      if ((subflowDetails && !flow.subflows[subflowDetails[1]]) || (!subflowDetails && !get(n.type))) {
