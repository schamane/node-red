...
import { util as redUtil, events } from '@node-red/util';
import credentials from '../nodes/credentials';
import Node from '../nodes/node.js';
import { Flow } from './flow.js';

console.debug('flow', util.inspect(Flow));
 ...
 */
export class Subflow extends Flow {
  subflowDef: any;
  subflowInstance: any;
  node_map: any;
...
function createNodeInSubflow(subflowInstanceId, def) {
  const node = clone(def);
  const nid = redUtil.generateId();
...
export function createSubflow(parent, globalFlow, subflowDef, subflowInstance) {
  return new Subflow(parent, globalFlow, subflowDef, subflowInstance);
}
export function createModuleInstance(type, parent, globalFlow, subflowDef, subflowInstance) {
