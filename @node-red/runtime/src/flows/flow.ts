        ...
        if (node.d !== true) {
          if (!node.subflow) {
            ...
            try {
              ...
              const subflow = createSubflow(this, this.global, subflowDefinition, node);
        ...
        if (group.env) {
          ...
          if (env) {
            ...
            if (type !== 'env' || value !== name) {
              ...
              try {
                const val = redUtil.evaluateNodeProperty(value, type, node, null, null);
                return [
                  {
                    val
                  },
        ...
        if (env) {
          ...
          if (type !== 'env' || value !== key) {
            ...
            try {
              ...
              const val = redUtil.evaluateNodeProperty(value, type, null, null, null);
              return val;
            } catch (e) {
              this.error(e);
            }
...
function handlePreDeliver(flow, sendEvent, reportError) {
  ...
  hooks.trigger('preDeliver', sendEvent, (err) => {
    if (err) {
      ...
      // postDeliver - the message has been dispatched to be delivered asynchronously (unless the sync delivery flag is set, in which case it would be continue as synchronous delivery)
      hooks.trigger('postDeliver', sendEvent, function (err1) {
        if (err1) {
