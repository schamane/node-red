 ...
 */
export function add(hookId, callback) {
  ...
  if (stackEntry2) {
    try {
      ...
      debug(`Unable to determined module when adding hook '${hookId}'. Stack:\n${stackEntries.join('\n')}`);
  ...
  debug(`Adding hook '${hookId}' from ${callModule}`);
 ...
 */
export function remove(hookId) {
  const [id, label] = hookId.split('.');
  if (!label) {
    throw new Error('Cannot remove hook without label: ' + hookId);
  }
  debug(`Removing hook '${hookId}'`);
...
function removeHook(id, hookItem) {
  const previousHook = hookItem.previousHook;
  const nextHook = hookItem.nextHook;
...
function invokeStack(hookItem, payload, done) {
  function callNextHook(err?: Error) {
    if (!hookItem || err) {
      done(err);
      return;
    }
