...
import { log, i18n, CustomErrorWithCode } from '@node-red/util';
...
export function init(_settings) {
  ...
  loadAllowList = parseModuleList(loadAllowList);
  loadDenyList = parseModuleList(loadDenyList);
}
...
function getLocalNodeFiles(dir, skipValidNodeRedModules?) {
  ...
  // TODO: check if it works properly async
  files.forEach(function (fn) {
    ...
    if (stats.isFile()) {
      ...
      if (!/^(\..*|lib|icons|node_modules|test|locales)$/.test(fn)) {
        const subDirResults = getLocalNodeFiles(path.join(dir, fn), skipValidNodeRedModules);
...
function scanDirForNodesModules(dir, moduleName, packageDetails?) {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      } else if ((isNodeRedModule || !moduleName || fn === moduleName) && isIncluded(fn) && !isExcluded(fn)) {
        try {
...
function scanTreeForNodesModules(moduleName?) {
  ...
  if (coreNodesDir) {
    let up = path.resolve(path.join(coreNodesDir, '..'));
    while (up !== coreNodesDir) {
      const pm = path.join(coreNodesDir, 'node_modules');
      if (pm !== userDir) {
...
export function getNodeFiles(disableNodePathScan) {
  ...
  const coreNodeEntry: Record<string, unknown> = {
    name: 'node-red',
    version: settings.version,
    nodes: {},
    icons: iconList
...
export function getModuleFiles(module) {
  // Update the package list
  const moduleFiles = scanTreeForNodesModules(module);
  if (moduleFiles.length === 0) {
    const err = new CustomErrorWithCode(log._('nodes.registry.localfilesystem.module-not-found', { module }));
  ...
  try {
    ...
    if (result.package) {
      ...
      if (result.isNodeRedModule) {
        ...
        result.allowed = checkModuleAllowed(pkg.name, pkg.version, loadAllowList, loadDenyList);
      }
    }
  } catch (err) {
